(function() {
  ...
  class Pawn extends Piece {
    ...
    get availableMoves() {
      ...
      if(position.row == 1 || position.row == 6)
        moves.push(board.getCoordinate([position.row + 2 * step, position.column]));

      moves = moves.filter(m => m !== null && m.piece === null);
  ...
  class Knight extends Piece {
    ...
    get availableMoves() {
      ...
      return iterateMove(this, directions, 1);
    }
  }
  ...
  class Move {
    ...
    constructor(board, notation) {
      ...
      if(notation.includes('-')) {
        ...
        // if(this.#to === null) {
        //   console.error([castling.king.position.rank, castling.side == 'K' ? FILES.KN : FILES.QB]);
        // }
      ...
      if(isCapturing) {
        if(this.#to.piece === null && from.piece instanceof Pawn) {
          // En passant
          let square = to.file + (this.#turns == 'b' ? '4' : '5');
